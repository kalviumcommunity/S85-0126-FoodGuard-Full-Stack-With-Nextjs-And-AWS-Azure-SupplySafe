...
export class DatabaseConfigValidator {
  ...
  static parseDatabaseUrl(databaseUrl: string): ValidationResult {
    ...
    const pgUrlPattern = /^postgresql:\/\/([^:]+):([^@]+)@([^:]+):(\d+)\/([^?]+)(\?.*)?$/;
    ...
    if (!host || host.trim() === '') {
      ...
      if (!config.ssl) {
        warnings.push('AWS RDS connections should use SSL (add ?sslmode=require)');
      }
    ...
    if (isNaN(port) || port <= 0 || port > 65535) {
      ...
      warnings.push(`Using non-standard port ${port} (default is ${this.DEFAULT_PORT})`);
    }
    ...
    } else if (password.toLowerCase() === 'password' || password.toLowerCase() === 'admin') {
      ...
      try {
        ...
        return {
          ...
          serverInfo: {
            ...
            currentConnections: parseInt(connectionResult.rows[0].current_connections, 10),
          },
        };
      } finally {
        client.release();
  ...
  static validateEnvironment(): ValidationResult {
    ...
    if (isProduction) {
      ...
      if (process.env.DATABASE_URL?.includes('password')) {
        warnings.push('Consider using secrets management for production credentials');
      }
    }
   ...
   */
  static generateConnectionSuggestions(config: Partial<DatabaseConfig>): string[] {
    const suggestions: string[] = [];

    if (config.host?.includes('.rds.amazonaws.com')) {
      suggestions.push(`postgresql://${config.username || 'admin'}:YourPassword@${config.host}:5432/${config.database || 'supplysafe'}?sslmode=require`);
    } else if (config.host?.includes('.postgres.database.azure.com')) {
      suggestions.push(`postgresql://${config.username || 'adminuser'}@${config.host}:YourPassword@${config.host}:5432/${config.database || 'supplysafe'}?sslmode=require`);
    } else if (config.host?.includes('localhost')) {
      suggestions.push(`postgresql://postgres:password@localhost:5432/${config.database || 'supplysafe'}`);
    } else {
      suggestions.push(`postgresql://${config.username || 'username'}:password@${config.host || 'your-db-host'}:5432/${config.database || 'supplysafe'}`);
    }
  ...
  static getProviderRecommendations(host?: string): string[] {
    ...
    if (host.includes('.rds.amazonaws.com')) {
      ...
      recommendations.push('Consider Azure Database for PostgreSQL - Hyperscale');
    }
