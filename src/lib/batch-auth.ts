...
export function requireBatchAuth(action: 'create' | 'view' | 'edit' | 'delete' | 'track') {
  ...
  switch (action) {
    case 'create':
      return { authorized: canCreateBatch(), reason: canCreateBatch() ? null : 'Insufficient permissions to create batches' }
    case 'view':
      return { authorized: canViewBatch(), reason: canViewBatch() ? null : 'Insufficient permissions to view batches' }
    case 'edit':
      return { authorized: canEditBatch(), reason: canEditBatch() ? null : 'Insufficient permissions to edit batches' }
    case 'delete':
      return { authorized: canDeleteBatch(), reason: canDeleteBatch() ? null : 'Only admins can delete batches' }
    case 'track':
      return { authorized: canTrackBatchMovement(), reason: canTrackBatchMovement() ? null : 'Insufficient permissions to track batch movements' }
    default:
