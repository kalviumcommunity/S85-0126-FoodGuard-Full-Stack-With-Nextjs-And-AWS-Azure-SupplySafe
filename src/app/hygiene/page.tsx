...
export default function HygienePage() {
  ...
  const overallStats = {
    ...
    averageScore: Math.round(hygieneReports.reduce((acc, r) => acc + r.overallScore, 0) / hygieneReports.length)
  }
    ...
    <AppShell>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Hygiene Reports</h1>
            <p className="text-gray-600 mt-1">Monitor and manage hygiene compliance across all kitchens</p>
          </div>
          <Button>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-amber-600">{overallStats.warning}</p>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Hygiene Inspection Reports ({filteredReports.length})</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <TableBody>
                  {filteredReports.map((report) => (
                    <TableRow key={report.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        <div>
                          <div className="text-sm font-medium text-gray-900">{report.kitchen}</div>
                          <div className="text-xs text-gray-500">{report.location}</div>
                        </div>
                      </TableCell>
                      <TableCell className="text-sm text-gray-600">{report.inspector}</TableCell>
                      <TableCell>
                        <div>
                      ...
                      <TableCell>
                        <div>
                          ...
                          <Progress value={report.overallScore} className="h-2 mt-1" />
                        </div>
                      </TableCell>
                      <TableCell>{getStatusBadge(report.status)}</TableCell>
                      <TableCell>{getPriorityBadge(report.priority)}</TableCell>
                      <TableCell>
                        <div className="text-sm text-gray-900">{report.issues}</div>
                      </TableCell>
                      <TableCell className="text-sm text-gray-600">{report.nextInspection}</TableCell>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">
                  Showing {filteredReports.length} of {hygieneReports.length} reports
                </span>
          ...
          <CardContent>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                {hygieneReports[2].categories && Object.entries(hygieneReports[2].categories).map(([category, score]) => (
              ...
              <div className="space-y-4">
                <h4 className="font-semibold text-gray-900">Key Issues Found</h4>
                <div className="space-y-2">
                  <div className="flex items-start space-x-2">
                    <AlertTriangle className="w-4 h-4 text-red-600 mt-0.5" />
                    <div>
                      <p className="text-sm font-medium text-gray-900">Storage temperature above safe limits</p>
                      <p className="text-xs text-gray-500">Critical - Immediate action required</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-2">
                    <AlertTriangle className="w-4 h-4 text-amber-600 mt-0.5" />
                    <div>
                      <p className="text-sm font-medium text-gray-900">Kitchen cleanliness below standards</p>
                      <p className="text-xs text-gray-500">Medium - Schedule deep cleaning</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-2">
                    <AlertTriangle className="w-4 h-4 text-amber-600 mt-0.5" />
                    <div>
                      <p className="text-sm font-medium text-gray-900">Equipment sanitation inadequate</p>
                      <p className="text-xs text-gray-500">Medium - Review cleaning protocols</p>
                    </div>
                  </div>
                </div>
