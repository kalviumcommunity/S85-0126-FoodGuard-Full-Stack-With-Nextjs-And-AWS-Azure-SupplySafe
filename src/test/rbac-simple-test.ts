...
function runTests() {
  ...
  test('Role Configuration - admin has all permissions', () => {
    return assert(
      JSON.stringify(roles.admin) === JSON.stringify(['create', 'read', 'update', 'delete']),
  ...
  test('Permissions - admin can create', () => {
    return assert(hasPermission('admin', 'create'), 'Admin should be able to create');
  });

  test('Permissions - editor cannot delete', () => {
    return assert(!hasPermission('editor', 'delete'), 'Editor should not be able to delete');
  });
  ...
  test('Utilities - getRolesWithPermission works correctly', () => {
    ...
    return assert(
      ...
      JSON.stringify(readRoles) === JSON.stringify(['admin', 'editor', 'viewer']),
  ...
  test('Audit Logging - logAccessCheck works', () => {
    ...
    return assert(
      Boolean(logOutput.includes('ALLOWED') && logOutput.includes('admin') && logOutput.includes('read')),
